...
ワークロードを再起動すると、ロードバランサーの IP アドレスが変更されます。IP アドレスを変更しない場合は、API でロードバランサーの IP アドレスを指定してください。
